...
class ParametersFixed():
    def __init__(self, therapy):
        ...
        if self._therapy == Therapies.MONO:
            self._annualTreatmentCost = Q.Zidovudine_COST
        else:
            self._annualTreatmentCost = Q.Zidovudine_COST + Q.Lamivudine_COST

        # transition probability matrix of the selected therapy
        self._prob_matrix = Q.TRANS_MATRIX
        ...
        if self._therapy == Therapies.COMBO:
            # treatment relative risk
            self._treatmentRR = Q.TREATMENT_RR
            # calculate transition probability matrix for the combination therapy
            self._prob_matrix = Q.Treatment_Matrix
